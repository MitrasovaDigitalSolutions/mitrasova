...
interface ServiceDocsLeftSidebarProps {
        ...
        <div className="space-y-1.5">
          {allServices.map((srv) => (
            <Link
              ...
              className={clsx(
                'flex items-center justify-between p-2.5 rounded-lg text-xs font-semibold transition-all',
                ...
                {catPosts.length > 0 ? (
                  catPosts.map((p) => (
                    <Link
                      ...
                      className={clsx(
                        'block text-xs py-1.5 px-2 rounded font-medium transition-all truncate',
                        p.slug === post.slug
                  ...
                  <span className="text-[11px] text-slate-400 italic block pl-2">Belum ada artikel</span>
                )}
              </div>
            );
          })}
