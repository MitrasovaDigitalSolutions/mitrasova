        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="space-y-2 text-center md:text-left">
            <span className="text-xs font-mono font-semibold text-cyan-400 uppercase tracking-wider">
              Pusat Panduan & Dokumentasi Resmi
            </span>
            <h3 className="text-2xl sm:text-3xl font-black text-white">
              Pelajari Cara Setup & Penggunaan {service.title}
            </h3>
            <p className="text-sm text-slate-300 max-w-xl">
              Akses panduan langkah demi langkah, rilis fitur baru, serta dokumentasi API teknis untuk tim engineer Anda.
            </p>
          </div>
          ...
          <Link href={`/docs/${service.slug}`} className="shrink-0 w-full sm:w-auto">
            <AppButton variant="secondary" size="lg" className="w-full sm:w-auto justify-center" rightIcon={<ChevronRight className="w-4 h-4" />}>
              ...
              <span>Buka Docs {service.title}</span>
            </AppButton>
          </Link>
        </div>
      </GlassCard>
